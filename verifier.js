...
jsonHeaders['X-AGENT-ID'] = roles.verifier.id

async function createRequest(id) {
  const requestUrl = `${config.credentials_api}/openid4vc/verification`
  const requestBody = {
    "request_credentials": [
      "PensionCredential"
    ],
...
*/
    ...
    verifierDidId: roles.verifier.identityId
  }
  const requestParams = {
    method: 'POST',
    headers: Object.assign(jsonHeaders, {
      ...
      // "successRedirectUri": `${config.credentials_api}/success?id=${id}`,
      // "errorRedirectUri": `${config.credentials_api}/error?id=${id}`,
...
async function showRequest(res) {
  const id = uuidv4()
  const cr = await createRequest(id)
  const requestURL = `openid-vc://?client_id=${encodeURIComponent(cr.clientId)}&request_uri=${encodeURIComponent(cr.authorizationRequestUri)}`
...
<html lang="en">
 <meta charset="UTF-8">
 <title>SICPA vastaanottaa eläkeläistodisteen</title>
 <style>
   *[lang]:not([lang="en"]) {
   display: none;
  }
